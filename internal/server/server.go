package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-grapevine/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-grapevine"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/articles",s.handleList)
    s.mux.HandleFunc("POST /api/articles",s.handleCreate)
    s.mux.HandleFunc("POST /api/articles/{id}/view",s.handleView)
    s.mux.HandleFunc("PUT /api/articles/{id}",s.handleUpdate)
    s.mux.HandleFunc("DELETE /api/articles/{id}",s.handleDelete)
