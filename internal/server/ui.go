...
async function load(){var r=await fetch(A+'/articles').then(function(r){return r.json()});items=r.articles||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.title||'').toLowerCase().includes(q)||(i.body||'').toLowerCase().includes(q)||(i.category||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No articles.</div>';return;}
var h='';f.forEach(function(i){
h+='<div class="item"><div class="item-top"><div style="flex:1">';
h+='<div class="item-title">'+esc(i.title)+'</div>';
...
if(i.tags)h+='<span>'+esc(String(i.tags))+'</span>';
if(i.slug)h+='<span>'+esc(String(i.slug))+'</span>';
if(i.helpful)h+='<span>'+esc(String(i.helpful))+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';
h+='</div>';});
document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/articles/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{title:'',body:'',category:'',tags:'',slug:'',helpful:'',not_helpful:'',status:''};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' ARTICLE</h2>';
h+='<div class="fr"><label>Title *</label><input id="f-title" value="'+esc(i.title||'')+'" ></div>';
h+='<div class="fr"><label>Body</label><input id="f-body" value="'+esc(i.body||'')+'" ></div>';
...
h+='<div class="fr"><label>Helpful</label><input id="f-helpful" value="'+esc(i.helpful||'')+'" ></div>';
h+='<div class="fr"><label>Not Helpful</label><input id="f-not_helpful" value="'+esc(i.not_helpful||'')+'" ></div>';
...
async function submit(){var body={title:document.getElementById('f-title').value.trim(),body:document.getElementById('f-body').value.trim(),category:document.getElementById('f-category').value.trim(),tags:document.getElementById('f-tags').value.trim(),slug:document.getElementById('f-slug').value.trim(),helpful:document.getElementById('f-helpful').value.trim(),not_helpful:document.getElementById('f-not_helpful').value.trim(),status:document.getElementById('f-status').value.trim()};
if(editId){await fetch(A+'/articles/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/articles',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
